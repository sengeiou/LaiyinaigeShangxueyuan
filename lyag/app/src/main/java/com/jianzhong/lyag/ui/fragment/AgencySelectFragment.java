...
public class AgencySelectFragment extends BaseRecyclerViewFragment {
    ...
    private DepartmentSelAdapter mAgencySelectAdapter = null;
    private List<DepartmentModel> mAgencyModels = new ArrayList<>();
    private Map<String, DepartmentModel> map = new HashMap<>();
    private String key;

    public static AgencySelectFragment getInstance() {
        AgencySelectFragment fragment = new AgencySelectFragment();
        return fragment;
    }

    @Override
    protected RecyclerView.Adapter getRecyclerViewAdapter() {
        mAgencySelectAdapter = new DepartmentSelAdapter(mContext, mAgencyModels);
        return mAgencySelectAdapter;
    }

    @Override
    protected View initView(LayoutInflater inflater) {
        mRootView = inflater.inflate(R.layout.fragment_agency_select, null);
        return mRootView;
    }
    ...
    private void loadData() {
        for (int i = 0; i < GroupVarManager.getInstance().mContactList.size(); i++) {
            if (GroupVarManager.getInstance().mContactList.get(i).getIs_outermost().equals("1") && GroupVarManager.getInstance().mContactList.get(i).getIs_dealer().equals("1")) { //parent_branch_id为0 就是没有上级部门 这里只显示没有上级部门的
    ...
    private void setCompanyData(List<DepartmentModel> list) {
        if (!ListUtils.isEmpty(list)) {
            ...
            mAgencyModels.addAll(list);
            mAgencySelectAdapter.notifyDataSetChanged();
        }
        ...
        mSideBarList.setOnTouchingLetterChangedListener(new SideBar.OnTouchingLetterChangedListener() {
            ...
            public void onTouchingLetterChanged(String s) {
                //该字母首次出现的位置
                int position = mAgencySelectAdapter.getPositionForSection(s.charAt(0));
                if (position != -1) {
