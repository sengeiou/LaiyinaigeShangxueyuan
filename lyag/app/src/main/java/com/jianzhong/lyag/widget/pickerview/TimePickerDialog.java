...
//        if (mTimeWheel.getCurrentHour() == 0 || mTimeWheel.getCurrentMinute() == 0) {
//            int currentHour = mTimeWheel.getCurrentHour();
//        }

        if (mTimeWheel.getCurrentHour() == 0) {
            calendar.set(Calendar.HOUR_OF_DAY, 23);
        } else {
            calendar.set(Calendar.HOUR_OF_DAY, mTimeWheel.getCurrentHour() - 1);
        }

        if (mTimeWheel.getCurrentMinute() == 0) {
            calendar.set(Calendar.MINUTE, 59);
        } else {
            calendar.set(Calendar.MINUTE, mTimeWheel.getCurrentMinute() - 1);
        }
        ...
        if (mPickerConfig.mCallBack != null) {
            mPickerConfig.mCallBack.onDateSet(this, mCurrentMillSeconds, currentHour);
        }
        dismiss();
    }
