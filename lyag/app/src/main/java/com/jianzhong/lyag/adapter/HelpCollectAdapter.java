 ...
 */
public class HelpCollectAdapter extends CommonAdapter<CollectModel> {
    private int isEdit;
    public HelpCollectAdapter(Context context, List<CollectModel> datas,int isEdit) {
    ...
    protected void convert(ViewHolder holder, final CollectModel item, int position) {
        ...
        if(item.getHelp() != null){
            holder.setText(R.id.tv_title,item.getHelp().getTitle());
            holder.setText(R.id.tv_content,item.getHelp().getContent());
            holder.setText(R.id.tv_tag, CommonUtils.getTagStr(item.getHelp().getTag()));
            holder.setText(R.id.tv_like,item.getHelp().getLike_num());
            holder.setText(R.id.tv_comment,item.getHelp().getComment_num());
        }
        ...
        holder.setOnClickListener(R.id.ll_item, new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(isEdit == 1){
                    ...
                    EventBus.getDefault().post(AppConstants.TAG_HELP);
