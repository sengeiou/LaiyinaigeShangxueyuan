...
public class HelpDetailActivity extends ToolbarActivity {
    ...
    private void initListener() {
        //键盘收起的监听
        SoftKeyBoardListener.setListener(HelpDetailActivity.this, new SoftKeyBoardListener.OnSoftKeyBoardChangeListener() {
            @Override
    ...
    private void getNoticeDetail() {
        ...
        HttpRequest.getInstance().post(HttpConfig.URL_BASE + HttpConfig.URL_HELP_DETAIL, params, new HttpCallBack() {
            @Override
    ...
    private void initController() {
        ...
        if(AppUserModel.getInstance().getmUserModel().getMotion() != null &&
                !ListUtils.isEmpty(AppUserModel.getInstance().getmUserModel().getMotion().getOperation())){
...
//                    File file = new File(path);
    ...
    @OnClick({R.id.head_ll_back, R.id.iv_collect, R.id.iv_menu, R.id.ll_like, R.id.ll_comment, R.id.ll_asign, R.id.iv_keyboard, R.id.iv_voice_record, R.id.iv_file_add})
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.iv_menu:      //菜单栏
                PopWindowUtil.getInstance().showMenu(HelpDetailActivity.this, mIvMenu, CommonMethodLogic.getShareMenu(), new PopWindowUtil.OnMenuItemClickListener() {

                    @Override
                    public void OnMenuItemClickListener(CommonMenuModel item, int position) {
                        if (position == 0) { //编辑
                            Intent intent = new Intent(mContext, EditHelpActivity.class);
                            intent.putExtra("data", data);
                            startActivity(intent);
                        } else { //删除
                            new AlertDialog.Builder(HelpDetailActivity.this).setTitle("提示").
                                    setMessage("您确认删除该求助吗").
...
//                        File f = new File(outFilePathList.get(i));
    ...
    private void detele() {
        Map<String, Object> params = new HashMap<>();
        params.put("help_id", help_id);
        HttpRequest.getInstance().post(HttpConfig.URL_BASE + HttpConfig.URL_HELP_DEL, params, new HttpCallBack() {
            @Override
            public void onSuccess(String s) {
    ...
    class ViewHolder {
        ...
        public void onViewClicked(View view) {
            switch (view.getId()) {
                case R.id.tv_all:
                    Intent intent = new Intent(HelpDetailActivity.this, MemberConActivity.class);
                    intent.putExtra("title", "求助点赞的用户全部");
     ...
     * 获取通知未读人列表
     */
    private void getHelpUser() {
        Map<String, Object> params = new HashMap<>();
        params.put("help_id", help_id);
        HttpRequest.getInstance().post(HttpConfig.URL_BASE + HttpConfig.URL_HELP_LIKE_USER, params, new HttpCallBack() {
            @Override
            public void onSuccess(String s) {
                ...
                if (resultList.getCode() == HttpConfig.STATUS_SUCCESS && resultList != null) {
                    if (mAvatarAdapter == null) {
                        ...
                        if (ListUtils.isEmpty(mContactsModels)) {
                            ...
                            LinearLayoutManager ms = new LinearLayoutManager(HelpDetailActivity.this);
            ...
            public void onSuccess(String s) {
                ...
                if (resultList.getCode() == HttpConfig.STATUS_SUCCESS) {
                    ...
                    if (mCommentAdapter == null) {
                        mCommentAdapter = new CommentWithAdoptAdapter(mContext, mCommentModels, help_id, new OnCommentListener() {
                            @Override
