 ...
 */
public class CommentWithLikeAdapter extends CommonAdapter<CommentModel> {
    ...
    public CommentWithLikeAdapter(Context context, List<CommentModel> datas, String notice_id, OnCommentListener listener) {
        super(context, R.layout.item_comment_with_like, datas);
    ...
    protected void convert(final ViewHolder holder, final CommentModel item, final int position) {
        if(item.getUser() != null){
            ...
            if(StringUtils.isEquals(item.getUser().getUser_id(), AppUserModel.getInstance().getmUserModel().getUser_id())){
        ...
        holder.setOnClickListener(R.id.ll_item, new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("comment_type", AppConstants.TAG_NOTICE);
                intent.putExtra("top_id",item.getComment_id());
                intent.putExtra("commentModel",item);
                intent.putExtra("isAdopt",0);
