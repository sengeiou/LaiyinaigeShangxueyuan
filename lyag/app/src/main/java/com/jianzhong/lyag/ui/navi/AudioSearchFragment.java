 ...
 */
public class AudioSearchFragment extends BaseRecyclerViewFragment {
    ...
    private AudioSearchAdapter mAdapter;
    private List<AudioModel> mList = new ArrayList<>();

    public static AudioSearchFragment newInstance(String keyWord) {

        Bundle args = new Bundle();

        AudioSearchFragment fragment = new AudioSearchFragment();
    ...
    protected RecyclerView.Adapter getRecyclerViewAdapter() {
        mAdapter = new AudioSearchAdapter(mContext,mList);
        return mAdapter;
    }
    ...
    public void updateSearch(String tag) {
        if (tag.equals(AppConstants.TAG_AUDIO)) {
    ...
    private void getSearchFindAudio(){
        Map<String,Object> params = new HashMap<>();
        params.put("key_word",keyWord);
        params.put("p",pageIndex+"");
        HttpRequest.getInstance().post(HttpConfig.URL_BASE + HttpConfig.URL_SERCH_FIND_AUDIO, params, new HttpCallBack() {
            @Override
            public void onSuccess(String s) {
                ResultList<AudioModel> resultList = GsonUtils.json2List(s,AudioModel.class);
