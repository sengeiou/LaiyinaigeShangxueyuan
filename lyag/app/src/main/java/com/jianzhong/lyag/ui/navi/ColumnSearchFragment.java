...
public class ColumnSearchFragment extends BaseRecyclerViewFragment {
    ...
    private ColumnSearchAdapter mAdapter;
    private List<ColumnContentModel> mList = new ArrayList<>();

    public static ColumnSearchFragment newInstance(String keyWord) {

        Bundle args = new Bundle();

        ColumnSearchFragment fragment = new ColumnSearchFragment();
    ...
    protected RecyclerView.Adapter getRecyclerViewAdapter() {
        mAdapter = new ColumnSearchAdapter(mContext,mList);
        return mAdapter;
    }
    ...
    public void updateSearch(String tag) {
        if (tag.equals(AppConstants.TAG_COLUMN)) {
    ...
    private void getSearchFindColumn(){
        Map<String,Object> params = new HashMap<>();
        params.put("key_word",keyWord);
        params.put("p",pageIndex+"");
        HttpRequest.getInstance().post(HttpConfig.URL_BASE + HttpConfig.URL_SERCH_FIND_COLUMN, params, new HttpCallBack() {
            @Override
            public void onSuccess(String s) {
                ResultList<ColumnContentModel> resultList = GsonUtils.json2List(s,ColumnContentModel.class);
